...
> **与当前 v0.4.27 测试版同签名，可覆盖安装保留设置和配对。** v0.4.27 用户可在应用底部点「检查更新」，检查后下载 APK，并按系统提示安装。安装前会暂停动画，完成后重新开启无障碍。手动下载安装前请先暂停动画；早期版本遗留投影层时，重启手机后重新连接助手。GitHub v0.4.11 / v0.4.16 / v0.4.17 等异签名旧版需记录参数后重装，卸载会清除设置和配对。
...
- **检查更新**：从 GitHub Release 下载 APK，支持镜像换源、完整性与签名校验；全部下载失败时提供发布页链接。只在手动操作时检查。
