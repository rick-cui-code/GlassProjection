...
public final class UpdateTrust {
    ...
    public static int compareVersion(String left,String right){
        String[] a=version(left),b=version(right);
        for(int i=0;i<Math.max(a.length,b.length);i++){int result=Integer.compare(i<a.length?Integer.parseInt(a[i]):0,i<b.length?Integer.parseInt(b[i]):0);if(result!=0)return result;}return 0;
    }
    private static String[] version(String value){
        if(!value.matches("v?[0-9]{1,6}(\\.[0-9]{1,6}){1,3}"))throw new IllegalArgumentException("无法识别发布版本号");
