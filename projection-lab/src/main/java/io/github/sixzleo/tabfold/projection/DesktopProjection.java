...
final class DesktopProjection extends View {
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        smoothed=lastFrame==0?angle:ProjectionMath.followAngle(smoothed,angle,now-lastFrame);lastFrame=now;
        ...
        shader.setFloatUniform("tilt",(float)Math.toRadians(ProjectionMath.effectTilt(smoothed,inner,AnimationSettings.startAngle,blocked))*entrance);
        shader.setFloatUniform("crop",blocked?0:ProjectionMath.cropFraction(smoothed,inner,AnimationSettings.stretchPercent)*entrance);
